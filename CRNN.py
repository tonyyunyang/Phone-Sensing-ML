...
#         # RNN layer
#         rnn_input_size = 32 * 5  # Depends on the output of the conv layers
#         self.rnn = nn.LSTM(rnn_input_size, 64, num_layers=2, batch_first=True)
...
#         # Flatten features and add time dimension for RNN
#         batch_size, C, H, W = out.shape
#         out = out.view(batch_size, W, C*H)  # Flatten the H and W dimensions

#         # RNN layers
#         out, _ = self.rnn(out)

#         # Fully connected layer
#         out = self.fc(out[:, -1, :])  # Take the last output of the RNN

#         return out
# model = CRNN().to(device) # Move the model to GPU
